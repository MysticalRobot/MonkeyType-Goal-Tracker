...
// modifies the icon svg with the theme's hex colors and creates data URI 
export function getIconURI(theme: Theme): string {
  const mainColor = 'e2b714';
  ...
  const modifedIcon = icon
    .split(mainColor).join(theme.mainColor)
    .split(bgColor).join(theme.bgColor);
