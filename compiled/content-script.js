(() => {
  ...
  function validate(item, example) {
  ...
  var mainColor = "e2b714";
  var bgColor = "323437";
  var icon = `<svg width="48" height="48" xmlns="http://www.w3.org/2000/svg">
...
<rect transform="rotate(150, 13.1281, 5.12963)" rx="1" id="svg_44" height="4" width="1" y="3.12963" x="12.62807" stroke-width="0" stroke="#fff" fill="#e2b714"/>
  ...
  // content-script.ts
  var computeIconDataURI = (() => {
    const root_style = window.getComputedStyle(document.documentElement);
    return () => {
      const modifedIcon = icon.split(mainColor).join(root_style.getPropertyValue("--main-color").substring(1)).split(bgColor).join(root_style.getPropertyValue("--bg-color").substring(1));
