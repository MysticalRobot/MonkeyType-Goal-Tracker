...
async function updateIcon(request) {
    try {
        ...
        const mainColorValues = await response1.json();
        const backgroundColorValues = await response2.json();
        ...
        if (error instanceof UpdateIconError) {
            ...
            return new UpdateIconError(error.message, 'n/a');
        }
    }
}
